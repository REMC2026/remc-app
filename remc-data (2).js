...
(function (root) {
  ...
  // Comparar dois registos por texto nao serve: o Postgres nao guarda as
  // chaves pela ordem em que as escrevemos - o jsonb reordena-as. Comparado
  // por texto, um registo que voltou do servidor parece sempre diferente do
  // que la esta, e a app grava outra vez o que nao mudou, sem parar.
  function same(a, b) {
    if (a === b) return true;
  ...
  // -------------------------------------------------------------- rede ------
  // O fetch do browser tem de ser chamado agarrado a janela. Chamado solto
  // rebenta com "Illegal invocation" - e como isso e um erro de rede como
  // outro qualquer, a gravacao ficava a repetir para sempre sem nunca subir.
  ...
  function applyRemote(t, r) {
    ...
    if (have && !have._pending
        ...
        && have.updated_at === r.updated_at) return false;      // ja e esta
  ...
  var api = {
    ...
    save: function (t, rec, opts) {
      ...
      // Se nada mudou, nao se grava. Isto vale para todas as areas, nao so
      // para as variacoes: uma pagina que redesenha e volta a gravar a mesma
      // lista - o que elas fazem - deixa de acordar a rede por nada.
      // Uma linha que so existe neste aparelho tem de subir, mesmo que o
      // conteudo nao mude - senao ficava aqui para sempre.
      ...
      cache[t][id] = {
        ...
        _pending: true,
        _local: true            // ainda nao veio de la; limpa-se quando vier
      };
      touched(t);
    ...
    /** Compara dois registos sem depender da ordem das chaves. */
    same: same,
    ...
    /** Faz um exame completo e devolve o resultado em portugues. Serve para
     *  quando alguma coisa nao aparece do outro lado e nao se sabe porque. */
    diagnose: function () {
      var r = { configurado: !!(cfg.url && cfg.anonKey), tabelas: cfg.tables.slice(),
                comRede: online(), porEnviar: outbox.length, conflitos: conflicts.length,
                emCache: {}, problemas: [] };
      cfg.tables.forEach(function (t) { r.emCache[t] = api.all(t).length; });
      if (outbox.length) {
        r.primeiroDaFila = { tabela: outbox[0].table, id: outbox[0].id,
                             tentativas: outbox[0].tries || 0,
                             ultimoErro: outbox[0].lastError || '(ainda nenhum)' };
      }
      if (conflicts.length) r.detalheConflitos = conflicts.slice(0, 5);

      return doFetch('/rest/v1/' + (cfg.tables[0] || 'remc_variations') + '?select=id&limit=1')
        .then(function () { r.leitura = 'OK'; })
        .catch(function (e) {
          r.leitura = 'FALHOU: ' + ((e && e.message) || e);
          r.problemas.push('nao consegue ler a base de dados');
        })
        .then(function () {
          ...
          return doFetch('/rest/v1/rpc/remc_write', { method: 'POST', body: {
            ...
            p_data: { diagnostico: true }, p_version: null, p_project: null,
            p_state: null, p_kind: null, p_actor: 'diagnostico',
            p_write_id: id + '_w', p_delete: true
          }}).then(function () { r.escrita = 'OK'; })
            .catch(function (e) {
              var m = (e && e.message) || String(e);
              r.escrita = 'FALHOU: ' + m;
              if (/remc_write/.test(m) && /function|schema|404/i.test(m)) {
                r.problemas.push('a funcao remc_write nao existe - falta correr o 02-fixes.sql');
              } else if (/row-level security/i.test(m)) {
                r.problemas.push('o 02-fixes.sql nao correu ate ao fim');
              } else if (/Illegal invocation/i.test(m)) {
                r.problemas.push('o remc-data.js publicado e o antigo - volta a envia-lo');
              } else {
                r.problemas.push('a escrita nao passa: ' + m);
              }
            });
        })
        .then(function () {
          if (!r.problemas.length && r.leitura === 'OK' && r.escrita === 'OK') {
            r.veredicto = 'Esta tudo a funcionar deste lado.';
          } else {
            r.veredicto = 'Ha um problema: ' + r.problemas.join('; ');
          }
          return r;
        });
    },
